...
package object barneshut {
	...
	case class Fork(nw: Quad, ne: Quad, sw: Quad, se: Quad) extends Quad {
		...
		def insert(b: Body): Fork = {
			val newSe = if (b.x > centerX && b.y > centerY) se.insert(b) else se
			val newNe = if (b.x > centerX && b.y < centerY) ne.insert(b) else ne
			val newSw = if (b.x < centerX && b.y > centerY) sw.insert(b) else sw
			val newNw = if (b.x <= centerX && b.y <= centerY) nw.insert(b) else nw
	...
	case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: Seq[Body])
		...
		val (mass, massX, massY) = (
			...
			bodies.foldLeft(0f)(_ + _.x) / bodies.size,
			bodies.foldLeft(0f)(_ + _.y) / bodies.size
		)
	...
	class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
		...
		def +=(b: Body): SectorMatrix = {
			val x = math.min((math.abs(b.x) / sectorSize).toInt, sectorPrecision - 1)
			val y = math.min((math.abs(b.y) / sectorSize).toInt, sectorPrecision - 1)
		...
		def combine(that: SectorMatrix): SectorMatrix = {
			for (i <- matrix.indices) {
				that.matrix(i) = matrix(i).combine(that.matrix(i))
			}
			that
		}
